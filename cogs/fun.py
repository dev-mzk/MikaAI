...
class bottest(commands.Cog):
  ...
  async def ship(self, ctx, user1: discord.User, user2: discord.User):
    ...
    try:
      ...
      await ctx.send(content=f'mika ships you :DDD :0\n{user1.mention}{user2.mention}', embed=embed, file=file)
      os.remove(unique_filename)
    except Exception as e:
      logging.error(f"An error occurred: {e}")
      await ctx.send(f"An error occurred, If the problem persists report the bug on our [server](https://discord.gg/VU5GkWQmGp) or [github](https://github.com/dev-mzk/MikaAI/issues)")
