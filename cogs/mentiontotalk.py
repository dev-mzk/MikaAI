...
class mentiontotalk(commands.Cog):
  ...
  def mentiontotalk(self, msg, user, id):
    msg = msg.replace('<@1296629954636812400>', '')  # Main Bot
    msg = msg.replace('<@1283276819038212136>', '')  # DevTest Bot (Feel free to change for your own devtest)
  ...
  @commands.hybrid_command(name="talk", description="Talk to Mika using slash commands")
  @app_commands.allowed_installs(guilds=True,users=True)
  @app_commands.allowed_contexts(guilds=True,dms=True,private_channels=True)
  @app_commands.describe(message="The message you want to send to Mika")
